...
export function WatchlistManager({ market, stocks, news }: WatchlistManagerProps) {
  ...
  useEffect(() => {
    setWatchlist(getWatchlist(market));
  }, [market]);
          ...
          {filteredStocks.slice(0, 8).map((stock) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  if (selected) {
                    setWatchlist(toggleWatchlist(stock.ticker, market));
                  } else {
        ...
        {watchlistNews.length ? (
          <div className="grid gap-4">
            {watchlistNews.map((item) => (
              <NewsCard key={item.id} market={market} news={item} />
            ))}
          </div>
        ) : (
          <EmptyState
