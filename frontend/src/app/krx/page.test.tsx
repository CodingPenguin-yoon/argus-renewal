...
describe("krx market signal page route", () => {
  it("renders flat market signal subtabs with 종합 default", async () => {
    ...
    expect(screen.getByText("파생")).toBeInTheDocument();
    expect(screen.getByText("트리거")).toBeInTheDocument();
    expect(screen.getByText("상방 쪽 정합성이 높아진 구간입니다.")).toBeInTheDocument();
    expect(screen.getByText("외국인·기관 유입이 우세합니다.")).toBeInTheDocument();
