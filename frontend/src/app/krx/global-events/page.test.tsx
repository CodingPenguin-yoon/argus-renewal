...
describe("krx global events page route", () => {
  it("renders flat tabs with summary default and conditional earnings tab", async () => {
    ...
    const component = await KrxGlobalEventsPage();
    render(component);

    expect(screen.getByRole("tab", { name: "종합" })).toBeInTheDocument();
    expect(screen.getByRole("tab", { name: "핵심 이벤트" })).toHaveAttribute("href", "/krx/global-events?tab=highlights");
    expect(screen.getByRole("tab", { name: "다음 24시간" })).toHaveAttribute("href", "/krx/global-events?tab=next-24h");
    expect(screen.getByRole("tab", { name: "이번 주" })).toHaveAttribute("href", "/krx/global-events?tab=week");
    expect(screen.getByRole("tab", { name: "실적" })).toHaveAttribute("href", "/krx/global-events?tab=earnings");
  ...
  it("supports direct deep-link navigation to 글로벌 이벤트 > 다음 24시간", async () => {
    vi.mocked(getGlobalEventsTabData).mockResolvedValue(buildFixture({ includeEarnings: true }));
