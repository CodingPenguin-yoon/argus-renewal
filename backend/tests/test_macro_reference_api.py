...
def test_macro_reference_service_normalizes_file_payload(tmp_path: Path) -> None:
    ...
    assert [item["source"]["series_id"] for item in payload["items"]] == ["DGS10", "FEDFUNDS"]
    assert payload["items"][0]["label"] == "미국채 10년물"
    assert payload["items"][0]["change_value"] == 0.06
    assert payload["items"][0]["metadata"]["semantics"] == "daily_market_yield_percent"
    assert payload["items"][1]["label"] == "연방기금실효금리(월평균)"
    assert payload["items"][1]["metadata"]["series_id"] == "FEDFUNDS"
